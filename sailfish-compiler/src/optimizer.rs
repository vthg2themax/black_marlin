...
impl VisitMut for OptmizerImpl {
    fn visit_block_mut(&mut self, i: &mut Block) {
        ...
        for mut stmt in i.stmts.drain(..) {
            ...
            // check if previous statement is render_text! macro
            let mut previous_text = if let Some(previous_text) =
                results.last().and_then(get_rendertext_value_from_stmt)
            {
                results.pop();
                previous_text
            } else {
            ...
            *fl.body.stmts.last_mut().unwrap() = syn::parse2(quote! {
            ...
            let mut new_stmts = syn::parse2::<Block>(quote! {{
                __sf_rt::render_text!(__sf_buf, #previous_text);
                #stmt
